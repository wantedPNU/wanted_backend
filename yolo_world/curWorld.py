...
async def process_and_annotate_image(cnt,input_dir,model,classes,output_dir,inference_setting:InferenceSettings,file_name):
...
async def run_inference_on_video(video_path, model, inference_setting: InferenceSettings, file_name):
    """비디오에 대해 추론을 실행"""
    # 추출된 프레임 수
    ...
    for i in range(count):
        await process_and_annotate_image(i, os.path.join(input_dir, f"{file_name[0:-4]}의 {i}번째 프레임.jpg"), model, inference_setting.classes,output_dir, inference_setting, file_name)
...
async def run_inference(inference_settings: InferenceSettings):
    ...
    files_and_dirs = os.listdir("./yolo_world/input_video/sample_test/")
    file_count = len([f for f in files_and_dirs if os.path.isfile(os.path.join("./yolo_world/input_video/sample_test/", f))])
    print(file_count)
    file_names = [f for f in files_and_dirs if os.path.isfile(os.path.join("./yolo_world/input_video/sample_test/", f))]
    print(file_names)
    ...
    for file_name in file_names:    
        # 비디오에 대해 추론 실행
        total_frame_count += get_frames_count(os.path.join(os.getcwd(), f"./yolo_world/input_video/sample_test/{file_name}"),inference_setting.frame_interval)
    ...
    for file_name in file_names:    
        # 비디오에 대해 추론 실행
        await run_inference_on_video(os.path.join(os.getcwd(), f"./yolo_world/input_video/sample_test/{file_name}"), model, inference_setting, file_name)
...
#     for file_name in file_names:    
#         # 비디오에 대해 추론 실행
#         total_frame_count += get_frames_count(os.path.join(os.getcwd(), f"./sample_test/{file_name}"),inference_setting.frame_interval)
...
#     for file_name in file_names:    
#         # 비디오에 대해 추론 실행
#         await run_inference_on_video(os.path.join(os.getcwd(), f"./sample_test/{file_name}"), model, inference_setting, file_name)
